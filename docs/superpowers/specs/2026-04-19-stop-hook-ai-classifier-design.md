...
- 输出必须是单个 JSON 对象；
- 不允许 markdown 包裹；
- 若最终回复中存在 `TASHAN_COMPLETION_SIGNAL_BEGIN ... TASHAN_COMPLETION_SIGNAL_END`，则优先按该显式信号块判定；
- 显式信号块缺失时，才退回自然语言语义分类；
...
- 如果回复明确说还差 `live smoke`、`push`、`merge`、`remote setup`、人工确认或其他尾项未闭合，必须判定为 `false`
